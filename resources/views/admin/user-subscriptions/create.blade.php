            ...
            <div class="col-md-8 mx-auto">
               ...
               @include('admin.include.message')
                ...
                <div class="card ">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.user-subscriptions.store') }}" method="post" enctype="multipart/form-data" id="UserSubscriptionForm">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-md-6 col-12"> 
                                    <div class="form-group">
                                        ...
                                        <select required name="user_id" id="user_id" class="form-control select2">
                                            ...
                                            @foreach(App\Models\User::whereRoleIs('User')->get()  as $option)
                                ...
                                <div class="col-md-6 col-12"> 
                                    <div class="form-group">
                                        ...
                                        <label for="subscription_id">Subscription Plan <span class="text-danger">*</span></label>
                                        <select required   name="subscription_id" id="subscription_id" class="form-control select2">
                                            ...
                                            <option value="{{ $option->id }}" {{  old('subscription_id') == $option->id ? 'Selected' : '' }} data-to_date="{{ now()->addDays($option->duration)->format('Y-m-d') }}">{{  $option->name ?? ''}} | {{ $option->duration }} days | @if($option->price == 0)<span class="badge badge-primary">Free</span>@else {{ format_price($option->price) }}@endif</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-12"> 
                                    <div class="form-group {{ $errors->has('from_date') ? 'has-error' : ''}}">
                                        ...
                                        <input required class="form-control" name="from_date" type="date" id="from_date" value="{{ now()->format('Y-m-d') }}" min="{{ now()->format('Y-m-d') }}" placeholder="Enter From Date" >
                                    </div>
                                </div>                                                          
                                <div class="col-md-6 col-12"> 
                                    <div class="form-group">
                                        <label for="to_date" class="control-label">To Date<span class="text-danger">*</span></label>
                                        <input required class="form-control" name="to_date" type="date" id="to_date" value="{{ now()->format('Y-m-d') }}" placeholder="Enter To Date"  min="{{ now()->format('Y-m-d') }}">
                                    </div>
                                </div>
                                {{-- <div class="col-md-6 col-12"> 
                                    <div class="form-group">
