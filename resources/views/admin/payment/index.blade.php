    ...
    <div class="container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                ...
                @include('admin.setting.sitemodal', [
                    'title' => 'How to use',
                    'content' =>
        ...
        <div class="row">
            <div class="col-md-8 mx-auto">
                <div class="card card-484">
                    <form class="forms-sample ajaxForm" action="{{ route('admin.setting.store') }}" method="post"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="group_name" required value="payment_method">
                        <div class="card-header">
                            <h3>{{ __('Payment Configuration') }}</h3>
                        </div>
                        <div class="card-body">
                            <div id="accordion">
                                            ...
                                            <div class="switch-content mt-4">
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('Active') }}<span
                                                            class="text-danger">*
                                                            <a href="javascript:void(0);" title="@lang('admin/tooltip.stripe_payment_method_active')"><i
                                                                    class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label>
                                                    <div class="col-sm-9">
                                                        <input type="checkbox" value="1" name="services_stripe_active"
                                                            class="js-switch switch-input stripe"
                                                            @if (getSetting('services_stripe_active') == 1) checked @endif />
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('API KEY') }}<span
                                                            class="text-danger">*
                                                            <a href="javascript:void(0);" title="@lang('admin/tooltip.stripe_payment_method_api_key')"><i
                                                                    class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label>
                                                    <div class="col-sm-9">
                                                        <input type="text" pattern="[a-zA-Z]+.*"
                                                            title="Please enter first letter alphabet and at least one alphabet character is required."
                                                            name="api_stripe_key" class="form-control"
                                                            placeholder="Stripe API KEY" required
                                                            value="{{ getSetting('api_stripe_key') }}">
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('Secert') }}<span
                                                            class="text-danger">*
                                                            <a href="javascript:void(0);" title="@lang('admin/tooltip.stripe_payment_method_secert')"><i
                                                                    class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label>
                                                    <div class="col-sm-9">
                                                        <input type="password" name="api_stripe_secret" class="form-control"
                                                            placeholder=" Stripe API Secert" required
                                                            value="{{ getSetting('api_stripe_secret') }}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div id="collapseThree" class="collapse" aria-labelledby="headingThree"
                                        ...
                                        <div class="accordion-body">
                                            <div class="switch-content mt-4">
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('Active') }}
                                                        <a href="javascript:void(0);" title="@lang('admin/tooltip.razor_pay_payment_method_active')"><i
                                                                class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label>
                                                    <div class="col-sm-9">
                                                        <input type="checkbox" value="1"
                                                            name="services_razor_pay_active"
                                                            class="js-switch switch-input razorpay"
                                                            @if (getSetting('services_razor_pay_active') == 1) checked @endif />
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('Razor API KEY') }}<span
                                                            class="text-danger">*
                                                            <a href="javascript:void(0);" title="@lang('admin/tooltip.razor_pay_payment_method_api_razor_key')"><i
                                                                    class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label> </label>
                                                    <div class="col-sm-9">
                                                        <input type="text" pattern="[a-zA-Z]+.*"
                                                            title="Please enter first letter alphabet and at least one alphabet character is required."
                                                            name="api_razor_key" class="form-control"
                                                            placeholder="Razor API KEY"
                                                            value="{{ getSetting('api_razor_key') }}">
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label for="exampleInputUsername2"
                                                        class="col-sm-3 col-form-label">{{ __('Secert') }}<span
                                                            class="text-danger">*
                                                            <a href="javascript:void(0);" title="@lang('admin/tooltip.razor_pay_payment_method_secert')"><i
                                                                    class="ik ik-help-circle text-muted ml-1"></i></a>
                                                    </label></label>
                                                    <div class="col-sm-9">
                                                        <input type="password" name="api_razor_secret"
                                                            class="form-control" placeholder="Razor API Secert"
                                                            value="{{ getSetting('api_razor_secret') }}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
