...
class {{ $data['model'] }} extends Model @if(isset($data['media']) && count($data['media']) > 0)implements HasMedia @endif
{
    ...
    protected $casts = [
        {!! $html !!}
        ]; @endif @isset($data['bulk_activation_btn'])
    ...
    public function  {{ lcfirst(str_replace(' ', '',str_replace('Id','',ucwords(str_replace('_',' ',$item))))) }}(){
      return  $this->belongsTo({{ $data['fields']['table'][$index] }}::class,'{{ $item }}','id');
    } @elseif($data['fields']['input'][$index] == 'select' && str_contains(strip_tags((string)$data['fields']['comment'][$index]),":"))
