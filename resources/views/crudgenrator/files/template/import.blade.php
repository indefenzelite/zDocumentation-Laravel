...
class {{ $data['model'] }}Import implements ToModel, WithStartRow
{
    ...
    */
    public function model(array $row)
    {
        ...
        ${{ str_replace('_id','',$item) }} = {{ $data['fields']['table'][$index] }}::where(\DB::raw("CONCAT(first_name, ' ', last_name)"), 'like', '%' . $row[{{ $index + 1 }}] . '%')->first(); @else

        ${{ str_replace('_id','',$item) }} = {{ $data['fields']['table'][$index] }}::where('name',$row[{{ $index + 1 }}])->first(); @endif @endif @endif @endforeach
    
        if({{ $variable }}){    
            {{ $variable }}->update([ @foreach($data['fields']['name'] as $index => $item) @if(array_key_exists('import_'.$index,$data['fields']['options'])) @if($data['fields']['input'][$index] == 'select_via_table')
            ...
            return new {{ $data['model'] }}([ @foreach($data['fields']['name'] as $index => $item) @if(array_key_exists('import_'.$index,$data['fields']['options'])) @if($data['fields']['input'][$index] == 'select_via_table')
