            ...
            <div class="col-md-8 mx-auto">
               ...
               {{$data['atsign']}}include('admin.include.message')
                ...
                <div class="card ">
                    ...
                    <div class="card-body">
                        <form action="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.store') }}" method="post" enctype="multipart/form-data" class="ajaxForm" id="{{ $data['model'] }}Form">
                            {{$data['atsign']}}csrf
                            ...
                            @foreach($data['fields']['name'] as $index => $item)
                                ...
                                <div class="{{ $data['fields']['column']['column_'.$index] }} col-12"> {{-- Textarea --}}
                                    <div class="form-group">
                                        ...
                                        <textarea {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }} class="form-control" name="{{ $item }}" id="{{ $item }}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}">{{ $data['curlstart'] }} old('{{ $item }}')}}</textarea>
                                    </div>
                                </div>
                                @elseif($data['fields']['input'][$index] == 'decimal')

                                <div class="{{ $data['fields']['column']['column_'.$index] }} col-12"> {{-- decimal --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ ucwords(str_replace('_',' ',$item)) }}{!! array_key_exists('required_'.$index,$data['fields']['options']) ? '<span class="text-danger">*</span>' : '' !!}</label>
                                        <input {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }}  class="form-control" name="{{ $item }}" type="number" step="any" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}')}}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}">
                                    </div>
                                </div>
                                ...
                                <div class="{{ $data['fields']['column']['column_'.$index] }} col-12"> {{-- checkbox radio --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}"><br>
                                        ...
                                        <input  {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }} class="js-single switch-input" @if( $data['fields']['default'][$index] == 1) checked @endif {{$data['atsign']}}if(old('{{ $item }}'))  checked {{$data['atsign']}}endif name="{{ $item }}" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="1" > {{ ucwords(str_replace('_',' ',$item)) }}{!! array_key_exists('required_'.$index,$data['fields']['options']) ? '<span class="text-danger">*</span>' : '' !!}</label>
                                    </div>
                                </div>
                                ...
                                <div class="{{ $data['fields']['column']['column_'.$index] }} col-12"> {{-- file/img --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ ucwords(str_replace('_',' ',$item)) }}{!! array_key_exists('required_'.$index,$data['fields']['options']) ? '<span class="text-danger">*</span>' : '' !!}</label>
                                        <input  {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }} {{ array_key_exists('multiple_'.$index,$data['fields']['options']) ? 'multiple' : '' }} class="form-control" name="{{ $item }}_file" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}')}}" >
                                ...
                                @elseif($data['fields']['input'][$index] == 'hidden')
                                   ...
                                   <input {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }} class="form-control" name="{{ $item }}" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}',request()->get('{{ $data['fields']['default'][$index] }}'))}}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}" >
                                @else

                                <div class="{{ $data['fields']['column']['column_'.$index] }} col-12"> {{-- Input --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ucwords(str_replace('_',' ',$item)) }}{!! array_key_exists('required_'.$index,$data['fields']['options']) ? '<span class="text-danger">*</span>' : '' !!}</label>
                                        <input {{ array_key_exists('required_'.$index,$data['fields']['options']) ? 'required' : '' }} class="form-control" name="{{ $item }}" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}','{{ $data['fields']['default'][$index] }}')}}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}" >
                                    </div>
                                </div>
                                @endif
                            @endforeach
                            ...
                            <div class="col-md{{ $data['media']['col'][$index] ? '-'.$data['media']['col'][$index] : '' }} col-12"> {{-- file/img --}}
                                <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                    ...
                                    <img id="{{ $item }}" class="d-none mt-2" style="border-radius: 10px;width:100px;height:80px;"/>
                                </div>
                            </div>
                            @endforeach @endif
    ...
    <{{ $data['script'] }}>
        ...
        $('.ajaxForm').on('submit',function(e){
            ...
            if(typeof(response) != "undefined" && response !== null && response.status == "success"){
                $('.ajaxForm').attr('action','{{ $data['curlstart'] }} url("{{$data['slashviewpath'].$data['view_name'] }}/update")}}/'+response.id);
                history.pushState({},"Tests","{{ $data['curlstart'] }} url('{{$data['slashviewpath'].$data['view_name'] }}')}}"+'/edit/'+response.id);
            }
        })
        @foreach($data['fields']['name'] as $index => $item)
