 ...
 */
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <div>
                                ...
                                <span><a href="javascript:void(0);" class="btn-link active records-type" data-value="All" >All</a> | <a href="javascript:void(0);" class="btn-link records-type" data-value="Trash">Trash</a></span>  @endisset
                            </div>
                              
                            <form action="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.index') }}" method="GET" id="TableForm">
                                <input type="hidden" name="page" value="{{ $data['curlstart'] }} request()->get('page') }}">
                                <input type="hidden" name="trash" value="{{ $data['curlstart'] }} request()->get('trash') }}">
                                <input type="hidden" name="length" value="{{ $data['curlstart'] }} request()->get('length') }}">
                                <div class="d-flex justify-content-right">@if(!isset($data['date_filter']))
                                    ...
                                    <a href="javascript:void(0);" id="reset" data-url="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.index') }}" class="btn btn-icon btn-sm btn-outline-danger mr-2" title="Reset"><i class="fa fa-redo" aria-hidden="true"></i></a>@if(!isset($data['date_filter'])){{ commentOutEnd() }}@endif
                                    {{$data['atsign']}}if(auth()->user()->isAbleTo('{{$data['permissions']['add']}}'))
                                        <a href="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.create') }}" class="btn btn-icon btn-sm btn-outline-primary" title="Add New {{ $heading }}"><i class="fa fa-plus" aria-hidden="true"></i></a>
                                    {{$data['atsign']}}endif @isset($data['export_btn'])

                                    <a href="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.export') }}" class="btn btn-icon btn-sm btn-outline-success ml-2" title="Export {{ $heading }}"><i class="fa fa-download" aria-hidden="true"></i></a> @endisset @isset($data['import_btn'])

                                    <a href="javascript:void(0);" class="btn btn-icon btn-sm btn-outline-secondary ml-2" id="import" title="Import {{ $heading }}"><i class="fa fa-upload" aria-hidden="true"></i></a>@endisset
                                </div>            
                            </form>
                            <div class="">
                                <select name="action" class=" form-control select-action ml-2" id="action">
                                    <option value="">Select Action</option> @foreach($data['fields']['name'] as $index => $item)  @if($data['fields']['input'][$index] == 'select')
                                    {{$data['atsign']}}php
                                            $arr = [{!! (string)$data['fields']['comment'][$index] !!}];
                                    {{$data['atsign']}}endphp
                                    {{$data['atsign']}}foreach(getSelectValues($arr) as $key => $option) 
                                    <option value="{{ $item }}-{{ $data['curlstart'] }}$option }}">{{ $data['curlstart'] }} $option }}</option>
                                    {{ $data['atsign'] }}endforeach @elseif($data['fields']['input'][$index] == "checkbox" ||$data['fields']['input'][$index] == "radio")
                                    <option value="{{ $item }}-1">{{ $item }}</option>
                                    <option value="{{ $item }}-0">Not {{ $item }}</option>
                                    @endif @endforeach @isset($data['softdelete'])

                                    
                                    <option value="Restore" class="trash-option {{$data['atsign']}}if(request()->get('trash') != 1)  d-none {{$data['atsign']}}endif">Restore</option> 
                                    <option value="Move To Trash" class="trash-option {{$data['atsign']}}if(request()->get('trash') == 1)  d-none {{$data['atsign']}}endif">Move To Trash</option>  @else

                                    <option value="Move To Trash">Move To Trash</option> @endisset
    ...
    @isset($data['import_btn'])
         <!-- Modal -->
        <div class="modal fade" id="importXlsx" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Import {{ $indexheading }}</h5>
                    <div class="d-flex justify-content-end">
                        <a href="javascript:void(0);" class="btn btn-link excel-template" download=""><i class="ik ik-arrow-down"></i> Download Template</a>
                    </div>
                    ...
                    <div class="modal-body">
                        ...
                        <form action="" id="ImportForm" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input reuired type="file" class="form-control" name="file" accept=".xlsx,.xls,.csv">
    ...
    {{ $data['atsign'] }}include('admin.include.more-action',['actionUrl'=> "{{ $data['slashroutepath'] }}{{ strtolower($data['view_name']) }}",'routeClass'=>"{{ strtolower($data['view_name']) }}"])
