            ...
            <div class="col-md-8 mx-auto">
               ...
               {{$data['atsign']}}include('admin.include.message')
                ...
                <div class="card ">
                    ...
                    <div class="card-body">
                        <form action="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.update',{{ $variable }}->id) }}" method="post" enctype="multipart/form-data" id="{{ $data['model'] }}Form">
                            {{$data['atsign']}}csrf
                            <div class="row">
                            ...
                            <div class="col-md{{ $data['media']['col'][$index] ? '-'.$data['media']['col'][$index] : '' }} col-12"> {{-- file/img --}}
                                <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                    ...
                                    <a href="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.destroy-media',{{ $variable }}->id).'?media={{ $item }}' }}" style="position: absolute;" class="btn btn-danger "><i class="fa fa-trash"></i></a>
                                    {{$data['atsign']}}endif
                                </div>
                            </div>
                            @endforeach @endif
