                            ...
                            <tbody>
                                {{ $data['atsign'] }}if({{ $indexvariable }}->count() > 0)
                                    {{ $data['atsign'] }}foreach({{ $indexvariable }} as  {{ $variable}})
                                        <tr>
                                            @foreach ($data['fields']['name'] as $index => $item)@if($data['fields']['input'][$index] == 'select_via_table')<td>{{ $data['curlstart'] }}fetchFirst(App\Models\'{{ $data['fields']['table'][$index]  }}',{{ $variable}}['{{ $item }}'],'name','--')}}</td>
