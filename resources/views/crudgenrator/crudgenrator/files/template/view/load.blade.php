    ...
    <div class="card-body">
        <div class="d-flex justify-content-between mb-2">
            ...
            <div>
                ...
                <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                    @php $i = 1; @endphp @foreach (getKeysByValue('showindex',$data['fields']['options']) as $temp) @php $index = explode('_',$temp)[1]; $item = $data['fields']['name'][$index] @endphp
                ...
                @if(!isset($data['print_btn']))
                     {{ commentOutStart() }}
                @endif
                <a href="javascript:void(0);" id="print" data-url="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['view_name']}}.print') }}"  data-rows="{{ $data['curlstart'] }}json_encode({{ $indexvariable }}) }}" class="btn btn-primary btn-sm">Print</a>
                @if(!isset($data['print_btn']))
                         {{ commentOutEnd() }}
                @endif
        ...
        <div class="table-responsive">
            <table id="table" class="table">
                <thead>
                    <tr>
                        <th class="no-export"><input type="checkbox" class="mr-2 " id="selectall"  value="">Actions</th> 
                ...
                <tbody>
                    ...
                    {{ $data['atsign'] }}foreach({{ $indexvariable }} as  {{ $variable}})
                            <tr>
                                ...
                                @foreach (getKeysByValue('showindex',$data['fields']['options']) as $temp) @php $index = explode('_',$temp)[1];  $item = $data['fields']['name'][$index]  @endphp @if($data['fields']['input'][$index] == 'select_via_table')
