    ...
    Route::group([@if($data['controller_namespace'] != null)'namespace' => 'App\Http\Controllers\{{ $data['controller_namespace'] }}',@endif 'prefix' => '{{ $prefix }}','as' =>'{{$as}}.'], function () {
        ...
        Route::get('restore/{id}', ['uses' => '{{ $data['model'] }}Controller@restore', 'as' => 'restore']);@endisset @if(isset($data['media']) && count($data['media']) > 0)
        
        Route::get('delete-media/{{'{'. substr($variable, 1).'}' }}', ['uses' => '{{ $data['model'] }}Controller@destroyMedia', 'as' => 'destroy-media']); @endif @isset($data['export_btn'])
